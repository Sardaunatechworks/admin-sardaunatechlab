document.addEventListener('DOMContentLoaded', () => {
    ...
    async function testLogin(pwd) {
        ...
        try {
            const res = await fetch('/api/content', {
                ...
                body: JSON.stringify({ type: 'services', items: cmsData.services }) // send current (empty or stale) data just to test auth
            });
            return res.ok;
        } catch (e) {
            return false;
    ...
    function renderList(type, items, keys) {
        ...
        items.forEach((item, index) => {
            ...
            const previewHtml = keys.map(k => {
                if ((k === 'image' || k === 'icon') && item[k]) {
                    if (k === 'image') return `<img src="${item[k]}" class="w-full h-32 rounded-lg object-cover mb-3 border border-slate-100">`;
                    return `<div class="text-brand-gold mb-2"><i data-lucide="${item[k]}" class="w-8 h-8"></i></div>`;
                }
                const isTitle = k === keys[0];
                return `<div class="truncate ${isTitle ? 'font-bold text-brand-navy text-lg mb-1' : 'text-slate-500 text-sm'}">${item[k] || ''}</div>`;
            }).join('');
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const savePromises = categories.map(async (type) => {
                const res = await fetch('/api/content', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                ...
                if (!res.ok) {
                    ...
                    throw new Error(`${type}: ${errData.error || res.statusText}`);
                }
                return type;
            });
            ...
            if (failures.length === 0) {
                ...
                const errorMsg = failures.map(f => f.reason).join('\n');
                console.error('Save failures:', failures);
                alert(`Some updates failed:\n${errorMsg}`);
            }
        } catch (e) {
            console.error(e);
            alert("Network Error saving changes: " + e.message);
        } finally {
